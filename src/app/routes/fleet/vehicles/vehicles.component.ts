...
export class FleetVehiclesComponent implements OnInit {
      ...
      res => {
        if (res) {
          ...
          for (var i = 0; i < this.vehicleList.length; i++) {
            this.vehicleList[i].description =
              (!this.vehicleList[i].brand ? '-' : this.vehicleList[i].brand) +
              ' ' +
              (!this.vehicleList[i].model ? '-' : this.vehicleList[i].model) +
              ' ' +
              (!this.vehicleList[i].year ? '-' : this.vehicleList[i].year);

              this.vehicleList[i].personName = this.vehicleList[i].owner.names + " "+ 
              this.vehicleList[i].owner.lastNames
          }
        }
      },
      err => {
  ...
  edit(selected) {
    ...
    this.selected.owner = selected.owner._id;
